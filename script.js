...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.querySelectorAll('.pay-card').forEach(card=>{
    card.addEventListener('click',()=>{
      ...
      const info=document.getElementById('bankInfo'),txt=document.getElementById('bankInfoText');
      if(selectedPay.includes('COD')||selectedPay.includes('QRIS')){info.classList.remove('show');}
      else{txt.innerHTML=`ℹ️ <strong>${selectedPay}</strong><br>Dikonfirmasi Kak ${NAMA_ADMIN} via WhatsApp.`;info.classList.add('show');}
    });
  });
  updateCart();
  loadProvinsi();
