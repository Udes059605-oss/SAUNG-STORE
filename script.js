...
async function loadProducts() {
  ...
  const skeletonCard = `<div class="skeleton-card"><div class="skeleton-img"></div><div class="skeleton-body"><div class="skeleton-line short"></div><div class="skeleton-line medium"></div><div class="skeleton-line full"></div><div class="skeleton-line price"></div></div></div>`;
  grid.innerHTML = `<div class="skeleton-grid" style="grid-column:1/-1;">${Array(4).fill(skeletonCard).join('')}</div>`;
  try {
    ...
    PRODUCTS = data.map(p => ({
      ...
      feats: p.feats || [],
      ukuran: p.ukuran ? p.ukuran.split(',').map(s=>s.trim()).filter(Boolean) : [],
      warna: p.warna ? p.warna.split(',').map(s=>s.trim()).filter(Boolean) : [],
      stok: p.stok !== null ? p.stok : 999,
      imgs: [
...
/* ── GRID ── */
function renderGrid() {
  let list = activeFilter === 'Semua' ? [...PRODUCTS] : PRODUCTS.filter(p => p.cat === activeFilter);
  if(searchQuery) list = list.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.cat.toLowerCase().includes(searchQuery.toLowerCase()));
  if(sortOrder === 'low') list.sort((a,b) => a.price - b.price);
  else if(sortOrder === 'high') list.sort((a,b) => b.price - a.price);
  else if(sortOrder === 'newest') list.sort((a,b) => b.id - a.id);
  if (list.length === 0) {
  ...
  document.getElementById('productsGrid').innerHTML = list.map(p => {
    ...
    return `
    <div class="product-card ${habis?'card-habis':''}" data-id="${p.id}">
      <div class="card-img-wrap">
        ${habis ? '<div class="badge" style="background:#555;color:#fff">HABIS</div>' : (p.badge ? `<div class="badge ${bClass[p.badge]||''}">${bText[p.badge]||p.badge}</div>` : '')}
...
function openModal(id) {
  ...
  const stokInfo = curProd.stok === 0 ? '<div style="color:#e05252;font-size:13px;margin-bottom:8px;">⚠️ Stok Habis</div>' : (curProd.stok < 10 ? `<div style="color:#e8a030;font-size:13px;margin-bottom:8px;">⚡ Sisa ${curProd.stok} stok!</div>` : '');
  const ukuranHtml = curProd.ukuran && curProd.ukuran.length ? `
    <div class="m-options">
      <div class="m-opt-label">Pilih Ukuran:</div>
      <div class="m-opt-btns" id="ukuranBtns">
        ${curProd.ukuran.map(u=>`<button class="opt-btn" data-ukuran="${u}">${u}</button>`).join('')}
      </div>
    </div>` : '';
  const warnaHtml = curProd.warna && curProd.warna.length ? `
    <div class="m-options">
      <div class="m-opt-label">Pilih Warna:</div>
      <div class="m-opt-btns" id="warnaBtns">
        ${curProd.warna.map(w=>`<button class="opt-btn" data-warna="${w}">${w}</button>`).join('')}
      </div>
    </div>` : '';
  document.getElementById('mInfo').innerHTML = `
    <div>
      ...
      <div class="m-reviews-section">
        <div class="m-rev-title">⭐ Ulasan Pembeli</div>
        <div id="reviewsList"><div style="color:#7a7068;font-size:13px;">Memuat ulasan...</div></div>
        <div class="m-rev-form">
          <div class="m-rev-form-title">Tulis Ulasan</div>
          <input type="text" id="revNama" placeholder="Nama kamu" maxlength="50">
          <div class="rev-stars-wrap">
            <span class="rev-star" data-s="1">★</span>
            <span class="rev-star" data-s="2">★</span>
            <span class="rev-star" data-s="3">★</span>
            <span class="rev-star" data-s="4">★</span>
            <span class="rev-star" data-s="5">★</span>
          </div>
          <textarea id="revKomentar" placeholder="Ceritakan pengalamanmu dengan produk ini..." maxlength="300"></textarea>
          <button class="btn-rev-submit" id="btnSubmitReview">Kirim Ulasan</button>
        </div>
      </div>
    </div>
    <div class="m-btns">
      <button class="btn-m-cart" id="btnModalCart" ${curProd.stok===0?'disabled style="opacity:.5;cursor:not-allowed"':''}>${bagIconSVG} + Keranjang</button>
      <button class="btn-m-wa" id="btnModalWA" ${curProd.stok===0?'disabled style="opacity:.5;cursor:not-allowed"':''}>${waIconSVG} Beli Sekarang</button>
    </div>`;
  ...
  document.querySelectorAll('.rev-star').forEach(star => {
    star.addEventListener('mouseover', () => {
      document.querySelectorAll('.rev-star').forEach((s,i) => s.classList.toggle('active', i < +star.dataset.s));
    });
    star.addEventListener('mouseout', () => {
      document.querySelectorAll('.rev-star').forEach((s,i) => s.classList.toggle('active', i < selectedRating));
    });
    star.addEventListener('click', (e) => {
      e.stopPropagation();
      selectedRating = +star.dataset.s;
      document.querySelectorAll('.rev-star').forEach((s,i) => s.classList.toggle('active', i < selectedRating));
    });
  });
  ...
  // ukuran & warna toggle
  document.querySelectorAll('#ukuranBtns .opt-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      document.querySelectorAll('#ukuranBtns .opt-btn').forEach(b=>b.classList.remove('active'));
      btn.classList.add('active');
      curProd._selectedUkuran = btn.dataset.ukuran;
    });
  });
  document.querySelectorAll('#warnaBtns .opt-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      e.stopPropagation();
      document.querySelectorAll('#warnaBtns .opt-btn').forEach(b=>b.classList.remove('active'));
      btn.classList.add('active');
      curProd._selectedWarna = btn.dataset.warna;
    });
  });
...
async function loadReviews(productId) {
  ...
  if(!el) return;
  try {
    const data = await sbFetch(`reviews?product_id=eq.${productId}&order=created_at.desc&limit=10`);
    if(!data.length) { el.innerHTML = '<div style="color:#7a7068;font-size:13px;padding:8px 0;">Belum ada ulasan. Jadilah yang pertama!</div>'; return; }
    el.innerHTML = data.map(r => {
      const stars = '★'.repeat(r.rating)+'☆'.repeat(5-r.rating);
      const tgl = new Date(r.created_at).toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'});
      return `<div class="rev-item">
        <div class="rev-header">
          <span class="rev-nama">${sanitize(r.nama)}</span>
          <span class="rev-stars">${stars}</span>
          <span class="rev-tgl">${tgl}</span>
        </div>
        ${r.komentar?`<div class="rev-komentar">${sanitize(r.komentar)}</div>`:''}
      </div>`;
    }).join('');
  } catch(e) { el.innerHTML = '<div style="color:#7a7068;font-size:13px;">Gagal memuat ulasan.</div>'; }
}
...
async function submitReview() {
  ...
  if(!selectedRating) { showToast('⚠️ Pilih bintang dulu!'); return; }
  ...
  try {
    await sbFetch('reviews', {
      ...
      body: JSON.stringify({ product_id: curProd.id, nama, rating: selectedRating, komentar: komentar || null })
    });
    showToast('Ulasan berhasil dikirim! ⭐');
    document.getElementById('revNama').value = '';
    document.getElementById('revKomentar').value = '';
    selectedRating = 0;
    document.querySelectorAll('.rev-star').forEach(s => s.classList.remove('active'));
    loadReviews(curProd.id);
  } catch(e) { console.error('Review error:', e); showToast('Gagal kirim ulasan: ' + e.message); }
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const searchEl = document.getElementById('searchInput');
  const sortEl = document.getElementById('sortSelect');
  if(searchEl) searchEl.addEventListener('input', () => { searchQuery = searchEl.value; renderGrid(); });
  if(sortEl) sortEl.addEventListener('change', () => { sortOrder = sortEl.value; renderGrid(); });
